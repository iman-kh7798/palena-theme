...
function story_gallery_add_metabox(){
    add_meta_box(
        'story_custom_gallery',
        'Gallery',
        'story_gallery_metabox_callback',
        'story', // Change post type name
        'normal',
        'core'
    );
}
add_action( 'admin_init', 'story_gallery_add_metabox' );


function story_gallery_metabox_callback(){
    ...
    if (idea_is_edit_page('new')){
        ...
        $gallery_data = get_post_meta( $postID, 'gallery_data', true );
    }
    ?>
...
function story_gallery_styles_scripts(){
    global $post;
...
add_action( 'admin_head-post.php', 'story_gallery_styles_scripts' );
add_action( 'admin_head-post-new.php', 'story_gallery_styles_scripts' );



function story_gallery_save( $post_id ) {
    ...
    // Correct post type
    if ( 'story' != $_POST['post_type'] ) // here you can set the post type name
        return;
...
add_action( 'save_post', 'story_gallery_save' );