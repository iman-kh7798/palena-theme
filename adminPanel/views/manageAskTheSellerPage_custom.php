...
if ( isset( $_REQUEST['askToRemove'] ) && ! empty( $_REQUEST['askToRemove'] ) ) {
	$resultToDelete = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}ask_the_seller_custom WHERE id = {$_REQUEST['askToRemove']}" );
	if ( $resultToDelete ) {
		$resultToDelete = $resultToDelete[0];

		$wpdb->delete( $wpdb->prefix . 'ask_the_seller_custom', array( 'id' => $_REQUEST['askToRemove'] ) );
	}
...
if ( isset( $_REQUEST['bim'] ) && ! empty( $_REQUEST['bim'] ) ) {
	...
	$wpdb->update( $wpdb->prefix . 'ask_the_seller_custom', array( 'seen'=> 'yes','OperatorDescription'=>sanitize_textarea_field($_REQUEST['OperatorDescription']),'updated_at' => $date
	), array( 'id' => $_REQUEST['bim'] ) );
}
if (isset($_REQUEST['reUpdate'])  && !empty($_REQUEST['reUpdate'])){
	$date = new DateTime("now", new DateTimeZone('Asia/Tehran') );
	$date=$date->format('Y-m-d H:i:s');
	$wpdb->update($wpdb->prefix.'ask_the_seller_custom', array('seen'=>'no','updated_at'=>$date), array('id'=>$_REQUEST['reUpdate']));

}
if ( isset( $_REQUEST['status'] ) && ! empty( $_REQUEST['status'] ) && $_REQUEST['status'] == 'seen' ) {
	$results = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}ask_the_seller_custom WHERE seen = 'yes' ORDER BY `id` DESC " );
	$seen    = 'seen';
} else {
	$results = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}ask_the_seller_custom WHERE seen = 'no' ORDER BY `id` DESC " );
	$seen    = 'not';
}
?>
<div class="wrap hh-font-family">
    <div style="display: inline-flex; align-items: center; justify-content: space-between; width: 100%; align-content: center">
		...
		<?php if ( $seen == 'not' ): ?>
            <form action="<?= menu_page_url( 'manageAskTheSellerPage_custom' ) ?>" method="get">
                <input type="hidden" name="page" value="ask_the_seller_custom">
                <input type="hidden" name="status" value="seen">
                <button type="submit" class="button-primary">Go to Verified Ask The Sellers Custom</button>
            </form>
		<?php else: ?>
            <form action="<?= menu_page_url( 'manageAskTheSellerPage_custom' ) ?>" method="get">
                <input type="hidden" name="page" value="ask_the_seller_custom">
                <input type="hidden" name="status" value="not">
                <button type="submit" class="button-primary">Go to Unverified Ask The Sellers Custom</button>
            </form>
		<?php endif; ?>
    </div>
	<?php
	if ( count( $results ) ):?>
        <table class="wp-list-table widefat plugins" style="margin-top: 20px;">
            ...
            <tr class="row-title">
                ...
                <th scope="col" colspan="2" class="manage-column column-primary"><span>dimensions_input</span></th>
                <th scope="col" colspan="2" class="manage-column column-primary"><span>length</span></th>
                <th scope="col" colspan="2" class="manage-column column-primary"><span>width</span></th>
                <th scope="col" colspan="2" class="manage-column column-primary"><span>height</span></th>
			...
			<?php foreach ( $results as $key => $result ): ?>
                <tr <?= $key % 2 ? '' : 'class="active"' ?> style="cursor: pointer" onclick="showRow('<?= $result->id ?>')">
                    ...
                    <td class="column-primary" colspan="2" ><span><?= $result->dimensions_input ?></span></td>
                    <td class="column-primary" colspan="2" ><span><?= $result->length ?></span></td>
                    <td class="column-primary" colspan="2" ><span><?= $result->width ?></span></td>
                    <td class="column-primary" colspan="2" ><span><?= $result->height ?></span></td>
        ...
        <tr id="row<?= $result->id ?>" class="row-shows <?= $key % 2 ? '' : ' active' ?>"  style="display: none">
            <td class="" colspan="28" style="width: 100%" >
			        ...
			        <?php if ( $seen == 'not' ): ?>
                        <form action="<?= menu_page_url( 'manageAskTheSellerPage_custom' ) ?>" method="get" style="display: flex">
                            <input type="hidden" name="page" value="ask_the_seller_custom">
                            <input type="hidden" name="status" value="not">
                    ...
                    <div class="hh-width100">
                        ...
                        <form action="<?= menu_page_url( 'manageAskTheSellerPage_custom' ) ?>" method="get" class="hh-width50">
                            <input type="hidden" name="page" value="ask_the_seller_custom">
                            <input type="hidden" name="status" value="seen">
